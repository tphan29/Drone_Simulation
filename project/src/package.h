...
class Package : public entity_project::Package, public EntityObservable {
 public:
    ...
    /** Increments the delivery time for the package (drone statistics).
     * @param time float
     */
    void IncreaseDeliveryTime(float time) { deliveryTime_ += time; }
    ...
    /** Returns the delivery time for the package.
     * @returns delivery time of the package as a float
     */
    float GetDeliveryTime() const { return deliveryTime_; }
