...
namespace csci3081 {
    ...
    void DroneSimulation::OutputDroneStats() {
        csci3081::CSVPrinter printer = csci3081::CSVPrinter("output/Statistics.csv");
        for (entity_project::Entity* e : entities_) {
            csci3081::Drone* drone = e->AsType<csci3081::Drone>();
            if (drone != nullptr) {
                // std::cout << std::endl << "Delivered package count: "
                // << drone->GetDeliveredPackagesCount() << std::endl;
                printer.AddData(0.0, static_cast<float>(drone->GetDeliveredPackagesCount()),
                    drone->GetTotalDistanceTraveled());
            }
        }
        printer.WriteToCSV();
    }
