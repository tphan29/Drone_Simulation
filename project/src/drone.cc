...
namespace csci3081 {
    Drone::Drone(const std::string& name, const picojson::array& position,
        ...
        radius_ = radius;
        deliveredPackagesCount_ = 0;
        totalDistanceTraveled_ = 0;
        details_ = object;
    ...
    void Drone::Update(float dt) {
        ...
        while (true) {
            if (!pendingPackages_.empty()) {
                ...
                if (customerDist <= radius_ + customer->GetRadius()) {
                    ...
                    // Output delivery time to the CSV file
                    CSVPrinter printer = CSVPrinter("output/Statistics.csv");
                    printer.AddData(double
                        ((*carriedPackages_.begin())->GetDeliveryTime()), 0.0, 0.0);
                    printer.WriteToCSV();
        ...
        totalDistanceTraveled_ += maxSpeed_ * dt;
